...
import require$$1$4, { app, dialog, BrowserWindow, ipcMain, shell } from "electron";
import { fileURLToPath } from "node:url";
import path$m from "node:path";
import fs$j from "node:fs/promises";
...
class SymbolIndexer {
  constructor() {
    ...
    this.cachePath = path$m.join(app.getPath("userData"), "symbol-cache.json");
    this.loadCache();
  }
  async loadCache() {
    try {
      const data = await fs$j.readFile(this.cachePath, "utf-8");
  ...
  async saveCache() {
    try {
      await fs$j.writeFile(this.cachePath, JSON.stringify(this.symbols));
    } catch (err) {
  ...
  async indexProject(rootPath) {
    ...
    try {
      ...
      for (let i = 0; i < allFiles.length; i += limit) {
        ...
        await Promise.all(chunk.map((file2) => this.extractSymbols(file2)));
      }
  ...
  async collectFiles(dir, fileList) {
    const entries = await fs$j.readdir(dir, { withFileTypes: true });
    for (const entry of entries) {
      const fullPath = path$m.join(dir, entry.name);
      if (entry.isDirectory()) {
        if (!IGNORE_DIRS.includes(entry.name)) {
          await this.collectFiles(fullPath, fileList);
        }
      } else if (entry.isFile()) {
        if (ALLOWED_EXTS.includes(path$m.extname(entry.name))) {
  ...
  async extractSymbols(filePath) {
    try {
      const content = await fs$j.readFile(filePath, "utf-8");
      ...
      lines.forEach((line, index) => {
        for (const { type: type2, regex } of regexps) {
  ...
  search(query) {
    ...
    const scored = this.symbols.map((s) => {
    ...
    }).filter((x) => x.score > 0).sort((a, b) => b.score - a.score || a.symbol.name.length - b.symbol.name.length).slice(0, 15);
...
function setupUpdater(mainWindow) {
  ...
  main$2.autoUpdater.logger.transports.file.level = "info";
  ...
  main$2.autoUpdater.on("download-progress", (progressObj) => {
  ...
  main$2.autoUpdater.on("update-downloaded", (info) => {
    ...
    dialog.showMessageBox({
      ...
      if (result.response === 0) {
        main$2.autoUpdater.quitAndInstall();
      }
    });
  });
  main$2.autoUpdater.checkForUpdatesAndNotify();
}
const __dirname$1 = path$m.dirname(fileURLToPath(import.meta.url));
process.env.APP_ROOT = path$m.join(__dirname$1, "..");
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
const MAIN_DIST = path$m.join(process.env.APP_ROOT, "dist");
...
ipcMain.handle("project:index", async () => {
  const root2 = path$m.join(process.env.APP_ROOT || "", "../..");
  return await indexer.indexProject(root2);
});
...
ipcMain.handle("file:read", async (_event, filePath) => {
  const root2 = path$m.join(process.env.APP_ROOT || "", "../..");
  if (!filePath.startsWith(root2)) {
    throw new Error("Access denied: File outside project root");
  }
  return await fs$j.readFile(filePath, "utf-8");
});
export {
  MAIN_DIST,
  RENDERER_DIST,
