...
export async function usageGuard(request: FastifyRequest, reply: FastifyReply) {
  const userId = (request as any).userId;
  const requestId = (request.body as any).clientRequestId || request.id;
  const type = request.url.includes("chat") ? "chat" : "edit";

  // 1. Identify Plan
  let plan = (request as any).plan || "free";
...
const usagePlugin: FastifyPluginAsync = async (app) => {
