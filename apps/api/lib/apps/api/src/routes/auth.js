...
const authRoutes = async (app) => {
    ...
    app.post("/auth/magiclink", {
        ...
        if (recentToken) {
            const err = new Error("Please wait a minute before requesting another link.");
            err.statusCode = 429;
            err.code = "RATE_LIMIT";
            throw err;
        }
    ...
    app.post("/auth/verify", {
        ...
        // Create session
        const accessToken = jwt.sign({ sub: user.id, email: user.email, plan: user.plan }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES_IN });
    ...
    app.post("/auth/refresh", {
        ...
        if (tokenRecord.revokedAt) {
            ...
            const err = new Error("Security alert: Token replay detected. All sessions revoked.");
            err.statusCode = 401;
            err.code = "UNAUTHENTICATED";
            throw err;
        }
        ...
        // Issue new ones
        const newAccessToken = jwt.sign({ sub: user.id, email: user.email, plan: user.plan }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES_IN });
